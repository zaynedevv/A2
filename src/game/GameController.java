...
public class GameController {
    ...
    private static final Map<String, Runnable> miscInputMap = new HashMap<>();
    private static final Map<String, Supplier<Double>> achievementMap = new HashMap<>();
    ...
    /**
     * Initializes the movement and achievement maps
     */
    private void setupCommands() {
        //Player Input
        miscInputMap.put("f", model::fireBullet);

        //Achievements
        achievementMap.put("Survivor", () -> calculateSurivorProgress());
        achievementMap.put("Enemy Exterminator", () -> calculateExterminatorProgress());
        achievementMap.put("Sharp Shooter", () -> calculateSharpShooterProgress());
    }
    ...
    public GameController(UI ui, GameModel model, AchievementManager achievementManager) {
        if (model == null || ui == null || achievementManager == null) {
     ...
     * @param input the keypress of the player
     */
    ...
    public void handlePlayerInput(String input) {
        ...
        Runnable isNonMovementInput = miscInputMap.get(input);

        if (isNonMovementInput != null) {
            isNonMovementInput.run();
        } else {
    ...
    public void renderGame() {
        ...
        ui.setStat("Time Survived", model.getPlayerStatsTracker().getElapsedSeconds() + " seconds");
        List<SpaceObject> objects = model.getSpaceObjects();
        objects.add(model.getShip());
        ui.render(objects);
    }

    /**
     * Returns the game model object
    ...
    /**
     * Calculate the survivor achievement progress
     * @return The survivor achievement progress
     */
    private double calculateSurivorProgress() {
        return Math.min(this.getStatsTracker().getElapsedSeconds() / 120.0, 1.0);
    }

    /**
     * Calculate the exterminator achievement progress
     * @return The exterminator achievement progress
     */
    private double calculateExterminatorProgress() {
        return Math.min(this.getStatsTracker().getShotsHit() / 20.0, 1.0);
    }

    /**
     * Calculate the sharp shooter achievement progress
     * @return The sharp shooter achievement progress
     */
    private double calculateSharpShooterProgress() {
    ...
    public void refreshAchievements(int tick) {
        ...
        for (Map.Entry<String, Supplier<Double>> entry : achievementMap.entrySet()) {
            String achievementName = entry.getKey();
            Supplier<Double> function = entry.getValue();

            double achievementValue = function.get();

            this.achievementManager.updateAchievement(achievementName, achievementValue);
        }

        for (Achievement a : this.achievementManager.getAchievements()) {
            ui.setStat(a.getName(), Double.toString(a.getProgress()));
        }

        if (this.isVerbose == true && tick % 100 == 0) {
            ui.log("Achievement Progress:");
            for (Map.Entry<String, Supplier<Double>> entry : achievementMap.entrySet()) {
                String achievementName = entry.getKey();
                Supplier<Double> function = entry.getValue();

                double achievementValue = function.get(); // calling the function

                ui.log("  " +  achievementName + ": " + achievementValue);
            }
        }
    ...
    private void showGameOverWindow() {
        ...
        sb.append(getStatsTracker().getElapsedSeconds());
        sb.append(" seconds\n");
